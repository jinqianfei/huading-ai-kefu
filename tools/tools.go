...
var Tools = map[string]Tool{
	...
	"get_huading_store_code": {
		Name:        "get_huading_store_code",
		Description: "根据客户侧门店代码/名称查询华鼎系统门店编号",
		Parameters: map[string]Parameter{
			"customer_id":       {Type: "string", Description: "客户ID", Required: true},
			"store_code_or_name": {Type: "string", Description: "客户门店代码或名称", Required: true},
			"address":           {Type: "string", Description: "门店地址（可选，用于辅助匹配）", Required: false},
		},
		Handler: GetHuadingStoreCode,
	},

	"get_warehouse_code": {
		Name:        "get_warehouse_code",
		Description: "根据客户ID和华鼎门店编码查询仓库字典编码",
		Parameters: map[string]Parameter{
			"customer_id":        {Type: "string", Description: "客户ID", Required: true},
			"huading_store_code": {Type: "string", Description: "华鼎门店编码", Required: true},
		},
		Handler: GetWarehouseCodeByStore,
	},

	"get_unit_type": {
		Name:        "get_unit_type",
		Description: "将客户单位转换为华鼎标准单位类型",
		Parameters: map[string]Parameter{
			"customer_unit": {Type: "string", Description: "客户侧单位（如箱、件、kg）", Required: true},
		},
		Handler: GetUnitType,
	},
}
...
// GetHuadingStoreCode 查询华鼎门店编号
func GetHuadingStoreCode(ctx context.Context, params map[string]interface{}) (interface{}, error) {
	customerID, _ := params["customer_id"].(string)
	storeCodeOrName, _ := params["store_code_or_name"].(string)
	...
	// 调用 store_mapping.go 中的函数
	storeCode, err := GetHuadingStoreCodeWithDetail(customerID, storeCodeOrName, address)
	if err != nil {
		return map[string]interface{}{
			"success":    false,
			"confidence": 0,
	...
	return map[string]interface{}{
		"success":           true,
		"huading_store_code": storeCode,
		"confidence":        0.95,
		"match_type":        "EXACT",
	}, nil
}

// GetWarehouseCodeByStore 查询仓库编码
func GetWarehouseCodeByStore(ctx context.Context, params map[string]interface{}) (interface{}, error) {
	customerID, _ := params["customer_id"].(string)
	huadingStoreCode, _ := params["huading_store_code"].(string)

	if customerID == "" || huadingStoreCode == "" {
		return nil, fmt.Errorf("customer_id and huading_store_code are required")
	}

	// 调用 warehouse_mapping.go 中的函数
	warehouseCode, err := GetWarehouseCodeByStore(customerID, huadingStoreCode)
	if err != nil {
		return map[string]interface{}{
			"success":    false,
			"confidence": 0,
...
// GetUnitType 获取单位类型
func GetUnitType(ctx context.Context, params map[string]interface{}) (interface{}, error) {
	customerUnit, _ := params["customer_unit"].(string)

	if customerUnit == "" {
		return nil, fmt.Errorf("customer_unit is required")
	}

	// 调用 unit_converter.go 中的函数
	unitType, err := GetUnitType(customerUnit)
	if err != nil {
		return map[string]interface{}{
			"success":    false,
			"confidence": 0,
